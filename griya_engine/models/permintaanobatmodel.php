...
<?php class permintaanobatmodel extends CI_Model {
function __construct()
{
   parent::__construct();
}

function get_permintaanobat($keyword = null, $page = 1){
	...
	$this->db->join('obat','permintaanobat.idobat = obat.id');
	$query = $this->db->get('permintaanobat');
	...
	$this->db->order_by('permintaanobat.tgl','DESC');
	$query = $this->db->get('permintaanobat');
	if ($query->num_rows() > 0){
		$result['rows'] = $query->result_array();	
	}
	
	log_message('error', var_export($result,true).'--permintaanobat-'.$this->db->last_query());
	return $result;
}
...
function search_permintaanobat($keyword = NULL){
  $result = array();
  if($keyword){
    $this->db->where('permintaanobat.id', $keyword);
  }
  $query = $this->db->get('permintaanobat');
  if($query->num_rows() > 0){
...
function get_next_id(){
	...
	$query1 = $this->db->get('permintaanobat');
...
function add($arrin){
	...
	$this->db->insert('permintaanobat', $arrin);
...
function edit($data){
	...
	$this->db->update('permintaanobat', $data); 
}

function get_data_by_id($idkat){
  $result = null;
  $this->db->where('permintaanobat.id',$idkat);
  $query = $this->db->get('permintaanobat');
  if($query->num_rows() > 0){
    $result = $query->row_array();
  }
...
function delete($idsubject){	
	$this->db->delete('permintaanobat', array('id' => $idsubject)); 
}

function reactivate($idsubject){	
	$this->db->where('id', $idsubject);
	$this->db->update('permintaanobat', array('activated'=>'1')); 
}

function revoke($idsubject){	
	$this->db->where('id', $idsubject);
	$this->db->update('permintaanobat', array('activated'=>'0')); 
