...
function __construct(){
	...
	if($this->tank_auth->is_logged_in() ){
        ...
        $this->allowed_level = $this->config->item('allowed_level_depo');
        ...
        $this->session->set_flashdata('adminfrom', '/stokobatdepo');
...
function browse($page = 1){
	...
	$data['is_stock'] = true;
	$data['link_stock'] = 'stokobatdepo/browse_stock/1/';
	...
	$data['input_list_add'] = array(
				...
				'metode' => array('type' => 'select', 'title' => 'Metode', 'value'=>'Masuk', 'select_list'=> array(
                                                                                                        'Masuk'=>'Masuk',
                                                                                                        'Keluar'=>'Keluar')),
	...
	//log_message('error', '$data[allrows]: '.var_export($allData['rows'],true));
	/*if($data['allrows']){
...
function browse_stock($page = 1, $idstock = NULL){
    ...
    $data['input_list_add'] = array(
                'tgl' => array('type' => 'text', 'title' => 'Tanggal', 'value'=> mdate('%d-%M-%Y'), 'class'=>'buttoncal','disabled'=>'true'),
                'obat' => array('type' => 'text', 'title' => 'Obat', 'value'=> 'Cari Disini','disabled'=>'true'),
    ...
    $data['addtitle'] = ucfirst(strtolower($this->title_)). ' :: '.$allData['rows'][0]['nama'];
    $data['edittitle'] = ' ';
    ...
    if($this->session->userdata('stokmode')){
        ...
        if($stokmode == 'waktu'){
            for ($i=0;$i<$data['rowcount'];$i++){
                //log_message('error','tgl: '.$data['allrows'][$i]['tgl'].' strtgl:'. strtotime($data['allrows'][$i]['tgl']).'>= strfrom:'. strtotime($keyfrom). 'keyform: '.$keyfrom .' --keyto: '.mdate('%Y-%m-%d', strtotime($keyto)).' strto:'. strtotime(mdate('%Y-%m-%d', strtotime($keyto)).' 24:00:00'). ' -tglto: '.$data['allrows'][$i]['tgl'].' strto:'. strtotime($data['allrows'][$i]['tgl']));
...
function last_stock($id){
    ...
    //log_message('error','stok data: '. var_export($allData,true));
    if($allData['rows']){
...
function save($action = 'add'){
    ...
    if($this->session->userdata('linkbrowse')){
  	...
  	if($action == 'add') {
  	    ...
  	    if($flashData['metode']=='Keluar'){
  	         $this->form_validation->set_rules('pemesan', 'Pemesan', 'trim|required|callback_search_check|xss_clean');
             $this->form_validation->set_rules('pengirim', 'Pengirim', 'trim|required|callback_search_check|xss_clean');
             $this->form_validation->set_rules('pengurus', 'Pengurus', 'trim|required|callback_search_check|xss_clean');
    ...
    if($action == 'add') {
        ...
        $flashData['obat'] = $this->input->post('obat');
        $flashData['idpemesan'] = $this->input->post('idpemesan');
        $flashData['idpengirim'] = $this->input->post('idpengirim');
        $flashData['idpengurus'] = $this->input->post('idpengurus');
        $flashData['tujuan'] = $this->input->post('tujuan');
        
        if($flashData['metode'] == 'Keluar'){
            if(($flashData['pemesan'] == 'Cari Disini') || ($flashData['pengirim'] == "Cari Disini") || ($flashData['pengurus'] == "Cari Disini")){
                $flashData['message'] = $flashData['message'].'<br/>Detil pengiriman harus diisi';
                $flashData['messageClass'] = "error";
            }
        }
        ...
        if(($flashData['metode'] == 'Keluar') && ($flashData['idobat'] != '')){
            if($flashData['debet'] > $this->last_stock($flashData['idobat'])){
                ...
                redirect('stokobatdepo/'.$linkbrowse.'/1/'.$idobat);     
            }
        }
  	...
  	if ($is_passed == FALSE){
    	...
    	if($action == 'add') {
        	...
        	$flashData['errors'] = array(
        		...
        		'ket'=>form_error('ket'),
        		'pemesan'=>form_error('pemesan'),
                'pengirim'=>form_error('pengirim'),
                'pengurus'=>form_error('pengurus')
        	);
  		...
  		if($action == 'add') {
  		        if($flashData['metode']=='Masuk'){
  		            $this->$titledb->add(array(
                        //'tgl'=> $flashData['tgl_tmp']. ' ' .mdate('%H:%i:%s', (time() - (3600 * 5))-3600*12),
                    ...
                    $this->$titledb->add(array(
                        ...
                        //'tgl'=> $flashData['tgl_tmp']. ' ' .mdate('%H:%i:%s', (time() - (3600 * 5))-3600*12),
                        'tgl'=> $flashData['tgl_tmp']. ' ' .mdate('%H:%i:%s'),
                        'kondisi'=> $flashData['kondisi'],
                        'pemesan'=> $flashData['idpemesan'],
                        'pengurus'=> $flashData['idpengurus'],
                        'pengirim'=> $flashData['idpengirim'],
                        'ket' => $flashData['tujuan']
                    ));
                    
                    
                    if($flashData['tujuan'] == "Kamar Obat"){
                        $this->load->model('historistokobatkamarobatmodel');
                        
                        $this->load->model('pegawaimodel');
                        $nama['pemesan'] = $this->pegawaimodel->get_pegawai_by_id($flashData['idpemesan']);
                        $nama['pengurus'] = $this->pegawaimodel->get_pegawai_by_id($flashData['idpengurus']);
                        $nama['pengirim'] = $this->pegawaimodel->get_pegawai_by_id($flashData['idpengirim']);
                    
                        $this->historistokobatkamarobatmodel->add(array(
                        //'tgl'=> $flashData['tgl_tmp']. ' ' .mdate('%H:%i:%s', (time() - (3600 * 5))-3600*12),
                        'tgl'=> $flashData['tgl_tmp']. ' ' .mdate('%H:%i:%s'),
                        'idobat'=> $flashData['idobat'],
                        'debet'=> NULL,
                        'kredit'=> $flashData['debet'],
                        'ket'=> '[<strong>Masuk dari Depo</strong> - <i>Pengirim:'.$nama['pengirim']['namapegawai'].' - Pemesan: '.$nama['pemesan']['namapegawai'].' - Pengurus: '.$nama['pengurus']['namapegawai'].' </i>] :: '. $flashData['ket']
                    ));
                    }
