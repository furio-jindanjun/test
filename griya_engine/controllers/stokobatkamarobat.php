...
<?php
class Stokobatkamarobat extends CI_Controller {

private $dheader = array();
private $allowed_level_admin = array();
private $editmode = false;
private $editiduser = null;
private $title = 'historistokobatkamarobat';
private $title_ = 'Stok Obat Kamar Obat';
private $title_url = 'stokobatkamarobat';  
private $titledb = 'historistokobatkamarobatmodel';
...
function __construct(){
	...
	if($this->tank_auth->is_logged_in() ){
        ...
        $this->session->set_flashdata('adminfrom', '/stokobatkamarobat');
...
function browse($page = 1){
	...
	$this->dheader['jsText'] = 'window.addEvent("domready", function(){
	                                       init_numonly();
                                            ...
                                            predict_obat = new MavSuggest.Request.JSON({
                                                "elem": "obat",
                                                "url":"'.base_url().'ajaxquery/obat_stok",
                                                "requestVar": "obat", 
                                                "singleMode": true,
                                            ...
                                            predict_editObat = new MavSuggest.Request.JSON({
                                                "elem": "editobat",
                                                "url":"'.base_url().'ajaxquery/obat_stok/edit",
                                                "requestVar": "editobat", 
                                                "singleMode": true,
	...
	$data['is_stock'] = true;
	$data['link_stock'] = 'stokobatkamarobat/browse_stock/1/';
    ...
    if($action == 'add') {
        ...
        if(($flashData['metode'] == 'Keluar') && ($flashData['idobat'] != '')){
                if($flashData['debet'] > $this->last_stock($flashData['idobat'])){
                    ...
                    log_message('error','$linkbrowse: '.$linkbrowse.' | obat: '.$flashData['idobat'].' | stok: '.$this->last_stock($flashData['idobat']));
                    redirect('stokobatkamarobat/'.$linkbrowse.'/1/'.$idobat);     
                }
        }
  		...
  		if($action == 'add') {
  		        if($flashData['metode']=='Masuk'){
                    ...
                    if($flashData['tujuan'] == "Apotik"){
                        ...
                        $this->historiklinikmodel->add(array(
                        //'tgl'=> $flashData['tgl_tmp']. ' ' .mdate('%H:%i:%s', (time() - (3600 * 5))-3600*12),
                        'tgl'=> $flashData['tgl_tmp']. ' ' .mdate('%H:%i:%s'),
                        'idobat'=> $flashData['idobat'],
                        'debet'=> NULL,
                        'kredit'=> $flashData['debet'],
                        'ket'=> '[<strong>Masuk dari Kamar Obat</strong> - <i>Pengirim:'.$nama['pengirim']['namapegawai'].' - Pemesan: '.$nama['pemesan']['namapegawai'].' - Pengurus: '.$nama['pengurus']['namapegawai'].' </i>] :: '. $flashData['ket']
                    ));
                    }
...
function id_check($str){
    $titledb = $this->titledb;
    log_message('error', 'bisa check');
    if($this->$titledb->get_data_by_id($str)){
        log_message('error', 'masuk check');
        return TRUE;
    }else{
        log_message('error', 'gak');
        $this->form_validation->set_message('id_check', '%s belum ada stok. <br/>Silahkan hubungi depo.');
        return FALSE;
    }
}
