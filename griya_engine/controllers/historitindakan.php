...
class Historitindakan extends CI_Controller {

private $dheader = array();
private $allowed_level_admin = array();
private $title = 'historitindakan'; 
...
function browse($id = NULL,$page = 1){
	...
	$data['columnHeaders'] = array(
							array('header_title'=>'Tanggal', 'field_name'=> 'tgl', 'class'=>'rowhead', 'width'=>'30%', 'rowinfo' => false),
							array('header_title'=>'Tindakan', 'field_name'=> 'obats', 'width'=>'70%', 'rowinfo' => false)
	);
	...
	if($allData['rows']){
	   if($data['keyword']){
	       foreach($allData['rows'] as $key1=>$val){
                        $obats = '[<strong>'.$allData['rows'][$key1]['nama'].'</strong> | '.$allData['rows'][$key1]['kode'].'] - ';
                        //log_message('error','$obat seaech: '.var_export($obats,true));
                        $allData['rows'][$key1]['obats'] = $obats;
            }
	   }
	   else{
	       foreach($allData['rows'] as $key=>$value){
              $obat = $this->$titledb->get_details_tindakan($allData['rows'][$key]['id']);
              $obats = '';
              log_message('error','$obat: '.var_export($obat,true));
              if($obat['tindakandata']){
                    foreach($obat['tindakandata'] as $key1=>$val){
                        $obats = $obats . '[<strong>'.$obat['tindakandata'][$key1]['nama'].'</strong> | '.$obat['tindakandata'][$key1]['kode'].'] - ';
