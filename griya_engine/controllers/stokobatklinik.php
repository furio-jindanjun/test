...
<?php
class stokobatklinik extends CI_Controller {
...
private $title_ = 'Stok Obat Klinik';
private $title_url = 'stokobatklinik';  
...
function __construct(){
	...
	if($this->tank_auth->is_logged_in() ){
        ...
        $this->session->set_flashdata('adminfrom', '/stokobatklinik');
...
function browse($page = 1){
	...
	$data['is_stock'] = true;
	$data['link_stock'] = 'stokobatklinik/browse_stock/1/';
	
	$data['url_add'] = $this->title_url."/save/add";
	$data['add_saveable'] = false;
	$data['url_edit'] = $this->title_url."/save/edit";
	$data['edit_saveable'] = false;
    ...
    if($this->session->userdata('stokmode')){
        ...
        if($stokmode == 'waktu'){
            for ($i=0;$i<$data['rowcount'];$i++){
                ...
                if((strtotime($data['allrows'][$i]['tgl']) >= strtotime($keyfrom))&&(strtotime($data['allrows'][$i]['tgl']) <= strtotime($keyto))){
    	...
    	if($action == 'add') {
    	   ...
    	   if(($flashData['metode'] == 'Keluar') && ($flashData['idobat'] != '')){
                if($flashData['debet'] > $this->last_stock($flashData['idobat'])){
                    ...
                    redirect('stokobatklinik/'.$linkbrowse.'/1/'.$idobat);     
                }
            }
